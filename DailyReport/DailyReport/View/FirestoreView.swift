                ...
                ScrollView {
                    VStack(spacing: 16) {
                        ReportFilterView(
                            ...
                            booths: firestoreManager.booths
                        )

                        ReportListView(
                            reports: firestoreManager.reports,
                            booths: firestoreManager.booths,
                            selectedLocationFilter: selectedLocationFilter,
...
struct ReportFilterView: View {
    ...
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 8) {
                Button(action: { selectedLocationFilter = nil }) {
                    Text("All")
                        .padding()
                        .frame(minWidth: 80)
                        .background(selectedLocationFilter == nil ? Color(hex: "#DB284E") : Color(.systemGray5))
                        .foregroundColor(.white)
                        .cornerRadius(10)
                }
                ...
                ForEach(["A", "B", "C"], id: \.self) { hall in
                    Button(action: {
                        ...
                        Text("Hall \(hall)")
                            ...
                            .frame(minWidth: 80)
                            .background(
                                selectedLocationFilter == hall ? Color(hex: "#DB284E") : Color(.systemGray5)
                            )
                            .foregroundColor(.white)
                            .cornerRadius(10)
                    }
...
struct ReportListView: View {
    let reports: [Report]
    let booths: [Booth]
    let selectedLocationFilter: String?
    let firestoreManager: FirestoreManager

    var body: some View {
        LazyVStack {
            ForEach(Array(filteredReports.enumerated()), id: \.element.id) { index, report in
                let reversedIndex = filteredReports.count - index - 1

                NavigationLink(
                    destination: ReportDetailView(report: report, firestoreManager: firestoreManager)
                ) {
                    ReportCardView(
                        report: report,
                        firestoreManager: firestoreManager,
                        reportIndex: reversedIndex 
                    )
                    .frame(width: 357, height: 173)
                    .cornerRadius(10)
                    .shadow(color: .black.opacity(0.2), radius: 4, x: 0, y: 4)
    ...
    var filteredReports: [Report] {
        if let selectedHall = selectedLocationFilter {
            let filteredBooths = booths.filter { $0.hall == selectedHall }
            let boothIDs = filteredBooths.map { $0.id }

            return reports
                .filter { boothIDs.contains($0.locationID) }
                .sorted(by: { $0.reportTime > $1.reportTime })
        } else {
            return reports.sorted { $0.reportTime > $1.reportTime }
        }
    }
}
